...
def ver_empresa(request, id):
    ...
    prosposta_investimentos = PropostaInvestimento.objects.filter(empresa=empresa).filter(status='PA')
    percentual_vendido = 0

    for pi in prosposta_investimentos:
...
def realizar_proposta(request, id):
    ...
    proposta_aceitas = PropostaInvestimento.objects.filter(empresa=empresa).filter(status='PA')

    total = 0
    for pa in proposta_aceitas:
